...
def d_axis_distill_loss(feature, target_feature):
    ...
    return distill_loss, torch.nn.functional.cosine_similarity(feature[:,:, :n], target_feature[:,:, :n], axis=-1).mean()

def t_axis_distill_loss(feature, target_feature, lambda_sim=1): 
    # print(feature.shape,target_feature.shape)
    n = min(feature.size(2), target_feature.size(2))
    l1_loss = torch.nn.functional.l1_loss(feature[:,:, :n], target_feature[:,:, :n], reduction='mean')
    sim_loss_t = - torch.log(torch.sigmoid(torch.nn.functional.cosine_similarity(feature[:,:, :n], target_feature[:,:, :n], axis=-1))).mean()
    distill_loss = l1_loss + lambda_sim * sim_loss_t
    return distill_loss