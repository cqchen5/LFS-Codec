...
def cal_stoi_gpu(ref_wav, deg_wav, sr):
    stoi_metric = ShortTimeObjectiveIntelligibility(fs=sr)
    stoi_score = stoi_metric(ref_wav, deg_wav)
    return stoi_score

def cal_pesq_gpu(ref_wav, deg_wav, sr):
    try:
        pesq_metric = PerceptualEvaluationSpeechQuality(fs=sr, mode='wb')
...
def calculate_stoi(ref_wav, deg_wav, sr):
    ...
    stoi_score = stoi(ref_wav.detach().numpy(), deg_wav.detach().numpy(), sr, extended=False)
    return stoi_score
...
def calculate_pesq(ref_wav, deg_wav, sr):
    ...
    try:
        wb_pesq_score = pesq(sr, ref_wav.detach().numpy(), deg_wav.detach().numpy(), 'wb')
        # breakpoint()
    except cypesq.NoUtterancesError:
        torchaudio.save('/home3/sppro/cqchen5/moshi_try/pad_toomuch.wav', ref_wav.unsqueeze(0).cpu(), 16000)
        # torchaudio.save('/home4/intern/cqchen5/moshi_try/NT.wav', deg_wav.detach().unsqueeze(0).cpu(), 16000)
        # breakpoint()
    # return nb_pesq_score, wb_pesq_score
    return wb_pesq_score
