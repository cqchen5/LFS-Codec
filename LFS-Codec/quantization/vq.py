...
class ResidualVectorQuantizer(BaseQuantizer):
    ...
    def __init__(
        ...
        self.distill_proj: torch.nn.Module
        ...
        if self.n_q == 1:
            self.distill_proj = torch.nn.Conv1d(    # For Distill
                self.dimension, 1024, 1, bias=False
            )
        # self.distill_proj = torch.nn.Conv1d(    # For Distill
        #         self.dimension, 1024, 1, bias=False
        #     )
    ...
    def forward(self, x: torch.Tensor, frame_rate: int):
        ...
        quantized_raw, codes, commit_loss, metrics = self.vq(x, n_q=n_q)    #[B,256,T]
        # print(commit_loss)
        # print(commit_loss.shape)
        B, _, _ = quantized_raw.shape
        if self.training and self.no_quantization_rate > 0:
            mask = (torch.rand(B, 1, 1, device=x.device) <= self.no_quantization_rate).float()
            quantized_raw = x * mask + (1 - mask) * quantized_raw
        quantized = self.output_proj(quantized_raw)                         #[B,512,T] This for Decoder

        # quantized_feature = self.distill_proj(quantized_raw)
        if self.n_q == 1:
            quantized_feature = self.distill_proj(quantized_raw)
        else:
            quantized_feature = torch.empty(0)
        codes = codes.transpose(0, 1)
        # codes is [B, K, T], with T frames, K nb of codebooks.
        bw = torch.tensor(n_q * bw_per_q).to(x)
        # breakpoint()
        return QuantizedResult(quantized, codes, bw, penalty=torch.mean(commit_loss), metrics=metrics), quantized_feature
    ...
    def forward(self, x: torch.Tensor, frame_rate: int):
        ...
        Returns:
            QuantizedResult: Quantized result with the following attributes:
                ...
                - `metrics` (dict): RVQ metrics, in particular rate of dead code replacement, and entropy.
        """
        semantic_result, feature_1 = self.rvq_first(x, frame_rate)
        # feature = semantic_result.x_feat    # For Distill
        # breakpoint()
        if self.n_q == self.n_q_semantic:
            return semantic_result,feature
        acoustic_result, feature_other = self.rvq_rest(x, frame_rate)
        ...
        # print("semantic_result.penalty",semantic_result.penalty)
        # print("acoustic_result.penalty",acoustic_result.penalty)
